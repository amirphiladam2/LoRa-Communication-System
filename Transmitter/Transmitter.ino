...
const unsigned long ALERT_MIN_INTERVAL = 5000UL;
...
void sendLoRaAlert(const char* type, float extraValue) {
  if (!loraReady) return;
  ...
  snprintf(payload, sizeof(payload), "%s,%.6f,%.6f,%.2f,Sats:%u", 
           type, currentLat, currentLng, extraValue, (unsigned int)gps.satellites.value());
...
void handleFlameDetection(unsigned long now) {
  static int fireCount = 0;
  bool isFlameActive = (digitalRead(FLAME_SENSOR_PIN) == (FLAME_SENSOR_ACTIVE_LOW ? LOW : HIGH));
  
  if (isFlameActive) fireCount++;
  else if (fireCount > 0) fireCount--;

  if (fireCount >= FLAME_SAMPLES_NEEDED && (now - lastFireAlert > ALERT_MIN_INTERVAL)) {
...
void handleQuakeDetection(unsigned long now) {
  ...
  if (shakeCount >= 4 && (now - lastQuakeAlert > ALERT_MIN_INTERVAL)) {
...
void loop() {
  ...
  handleFlameDetection(now);
  handleQuakeDetection(now);
  printStatus(now);
